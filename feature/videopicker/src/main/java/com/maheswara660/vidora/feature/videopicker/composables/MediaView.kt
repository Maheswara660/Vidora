...
import com.maheswara660.vidora.feature.videopicker.state.SelectionManager
...
fun MediaView(
    ...
    val loadedUris = remember { mutableSetOf<Uri>() }
            ...
            itemsIndexed(
                ...
                VideoItem(
                    ...
                    modifier = Modifier.onVisibilityChanged { isVisible ->
                        val videoUri = video.uriString.toUri()
                        if (isVisible && videoUri !in loadedUris) {
                            loadedUris.add(videoUri)
                            onVideoLoaded(videoUri)
                        }
                    },
                )
            }
