...
fun VidoraTheme(
    ...
    val isAmoled = themeConfig == ThemeConfig.AMOLED
    ...
    // Apply custom accent if specified and not Material You
    if (accentColorIndex >= 0) {
        val accent = if (accentColorIndex == 0) {
            // Teal (Default)
            AccentCombination(primaryLight, secondaryLight, "Teal")
        } else {
            CustomAccents.getOrNull(accentColorIndex - 1)
        }
